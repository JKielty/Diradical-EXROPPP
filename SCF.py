...
'''
File containing helper functions for running the SCF procedure in D-ExROPPP
'''
...
def re_center(coords, atoms, coords_h):
    ...
    '''
    com = np.zeros(3)
    masses = np.array([atoms[i][1] for i in range(len(atoms))], dtype=float)
    com = (masses[:, None] * coords_h).sum(axis=0) / masses.sum()
    coords -= com
    return com, coords
    ...
    for natom in range(natoms_n):
        ...
        for iatom in range(array_all.shape[0]):
            ...
            if distn < cutoff:
                nbonds+=1
        '''
        diffs = array_all[natom + natoms_c, :] - array_all       # shape (N, 3)
        dists = np.linalg.norm(diffs, axis=1)
        nbonds = int(np.sum(dists < cutoff)) - 1                 # subtract self
...
def v_term(dist_array,natoms_c,natoms_n,natoms,n_list,params):
    ...
    array=np.zeros_like(dist_array)
    # C-C repulsion
    i_idx, j_idx = np.triu_indices(natoms_c, k=1)
    vals = U / (1 + dist_array[:natoms_c, :natoms_c][i_idx, j_idx] / r0)
    array[:natoms_c, :natoms_c][i_idx, j_idx] = vals
    array[:natoms_c, :natoms_c][j_idx, i_idx] = vals
    np.fill_diagonal(array[:natoms_c, :natoms_c], U)
    # C-N repulsion
    for i in range (natoms_c):
        for j in range (natoms_c,natoms_c+natoms_n):
             if n_list[j-natoms_c]==0:
...
def density(orbs, ndocc):
    ...
    '''  
    return 2 * (orbs[:, :ndocc] @ orbs[:, :ndocc].T) + orbs[:, ndocc:ndocc+2] @ orbs[:, ndocc:ndocc+2].T  
...
def energy(hopping,repulsion,fock_mat,density,orbs,ndocc):
    ...
    """
    return 0.5 * np.einsum('ij,ij->', density, hopping + fock_mat)
