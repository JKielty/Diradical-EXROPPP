...
def evaluate_spectrum(plot_cmd, wavelength):
    ...
    arithmetic operation in the gnuplot expression (*, /, **, +, -) acts as
    a numpy ufunc and the entire spectrum is returned in one vectorised call.
    This is orders of magnitude faster than looping eval() per wavelength
    point, and works equally well for single-state and combined expressions.
    ...
    '''
    x = wavelength          # eval() below references this variable
...
def write_combined_dat(wavelength, broad_combined, molecule_name):
    '''
    Write the Boltzmann-weighted combined spectrum to a two-column .dat file.

    Format: whitespace-separated wavelength (nm) and intensity columns with a
    comment header. This is far more compact than a gnuplot Lorentzian expression
    file and loads instantly with np.loadtxt — no eval() required.

    Args:
        wavelength (ndarray): Wavelength array in nm.
        broad_combined (ndarray): Unnormalised combined spectrum intensities.
        molecule_name (str): Used to construct the output filename.
    Returns:
        outfile (str): Path to the written file.
    '''
    outfile = f"{molecule_name}_combined_spectrum.dat"
    np.savetxt(outfile,
               np.column_stack([wavelength, broad_combined]),
...
parser.add_argument('--no-normalise', dest='normalise', action='store_false',
                    help="Disable normalisation. Use when comparing spectra on an absolute scale.")
...
write_combined_dat(wavelength, broad_combined, args.molecule_name)
...
if args.expfile is not None:
    ...
    exp_abs = exp_abs / np.max(exp_abs)   # normalise to [0, 1] to match calculated curves
...
if args.normalise:
    ...
    ymax_vals = [np.max(broad_singlet[mask]),
                 np.max(broad_triplet[mask]),
                 np.max(broad_combined[mask])]
    if args.expfile is not None:
        exp_mask = (exp_wl >= args.xmin) & (exp_wl <= args.xmax)
        if exp_mask.any():
            ymax_vals.append(np.max(exp_abs[exp_mask]))
    ax.set_ylim(0, max(ymax_vals) * 1.05)
ax.legend()
