...
# Parse, evaluate, normalise, and plot each of the three SCF spectra
for gp_file, label, colour in zip(args.spectrum_files, SCF_LABELS, SCF_COLOURS):
    plot_cmd = parse_spectrum(gp_file)
    broad = evaluate_spectrum(plot_cmd, wavelength)
    broad = (broad - np.min(broad)) / (np.max(broad[mask]) - np.min(broad))
    ax.plot(wavelength, broad, color=colour, linewidth=2, label=label)
